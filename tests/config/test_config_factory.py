...
class TestDeployConfigLoading:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("minicpmo_4_5_3gpu_stage1_replicas.yaml", "0", "1,2", "0", 2),
            ("minicpmo_4_5_4gpu_stage1_replicas.yaml", "0", "1,2,3", "0", 3),
            ("minicpmo_4_5_8x4090_stage1_replicas.yaml", "0,1,2,3", "4,5,6,7", "5", 4),
        ],
    )
    def test_load_minicpmo_replica_deploy_configs(
        ...
        deploy_path = Path(get_deploy_config_path(filename))
        ...
        assert all(stage.yaml_engine_args["async_scheduling"] is False for stage in stages[:2])
        assert all("Async" not in (stage.scheduler_cls or "") for stage in stages[:2])
...
class TestMingFlashOmniPipeline:
    ...
    def test_tts_yaml_loads_and_merges(self):
        ...
        deploy_path = Path(get_deploy_config_path("ming_flash_omni_tts.yaml"))
        if not deploy_path.exists():
    ...
    def test_thinker_only_yaml_loads_and_merges(self):
        ...
        deploy_path = Path(get_deploy_config_path("ming_flash_omni_thinker_only.yaml"))
        if not deploy_path.exists():
    ...
    def test_image_yaml_loads_and_merges(self):
        ...
        deploy_path = Path(get_deploy_config_path("ming_flash_omni_image.yaml"))
        if not deploy_path.exists():
...
class TestAuraOmniDeploy:
    ...
    def test_aura_omni_deploy_resolves_four_native_stages(self):
        ...
        stages, _ = StageConfigFactory._create_legacy_from_registry(
            pipeline_cfg,
            cli_overrides={},
            deploy_config_path=get_deploy_config_path("aura_omni.yaml"),
        )
